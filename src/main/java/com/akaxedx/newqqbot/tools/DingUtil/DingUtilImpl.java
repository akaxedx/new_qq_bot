...
@Component
public class DingUtilImpl implements BaseUtil {


    private final Logger logger = LoggerFactory.getLogger(DingUtilImpl.class);
    @Autowired
    ...
    public String doThis(MessageEvent event, Bot bot) {
        ...
        switch (event.getMessage().split("\\[CQ:at,qq=|]")[0]){
            case DingConstant.DING_GET -> {
                reply = getDing(gEvent);
            }
            case DingConstant.DING_CULTIVATE -> {
                reply = cultivate(gEvent);
            }
            case DingConstant.DING_BATTLE -> {
                reply = battle(gEvent, bot);
            }
            case DingConstant.DING_BANG -> {
                QueryWrapper<Ding> queryWrapper = new QueryWrapper<>();
                queryWrapper.lambda()
                        .eq(Ding::getGroupId, gEvent.getGroupId())
                        .eq(Ding::getHas,DingConstant.HAS_DING)
                        .orderBy(true,false,Ding::getLength);
    ...
    private String getDing(GroupMessageEvent event) {


        String reply = BaseReplyConstant.DING_GET_NULL;

            QueryWrapper<Ding> queryWrapper = new QueryWrapper<>();
            queryWrapper.lambda().eq(Ding::getId, event.getUserId()).eq(Ding::getGroupId,  event.getGroupId());
            List<Ding> dings = dingMapper.selectList(queryWrapper);
            Random r = new Random();
            Double length = r.nextDouble() * 2 + 3;

            // 没有金箍棒
            Ding newDing = new Ding(event.getUserId(), length, 1, 0L, event.getGroupId());
            if (dings.isEmpty()) {
    ...
    private String cultivate(GroupMessageEvent event) {
        ...
        String reply = BaseReplyConstant.NO_DING;
            ...
            // 领取了
            reply = cLength(dings.get(0), length, lucky, event.getSender().getNickname());
        return reply;
    }
    private String battle(GroupMessageEvent event, Bot bot) {
        ...
        String reply;
            ...
            if (split.length == 1) {
                ...
                queryWrapper1.lambda().eq(Ding::getHas, DingConstant.HAS_DING).eq(Ding::getGroupId, event.getGroupId());
                ...
                if (allDingList.isEmpty()) {
                    ...
                    reply = bLength(event.getSender().getNickname(), battleName, lucky, alive, thisUser, length, battleUser, bot, event.getGroupId());
                }
            } else {
                logger.info("进入分流");
                QueryWrapper<Ding> queryWrapper1 = new QueryWrapper<>();
                queryWrapper1.lambda().eq(Ding::getId, split[1]).eq(Ding::getGroupId, event.getGroupId());
    ...
    private String bLength(String thisName,String battleName, boolean lucky, boolean alive, Ding thisUser, double length, Ding battleUser,Bot bot,Long groupId) {
        ...
        if (battleLength >= 30) {
            UpdateWrapper<Ding> updateWrapper = new UpdateWrapper<>();
            updateWrapper.lambda().eq(Ding::getGroupId, groupId).set(Ding::getHas,0L);
            dingMapper.update(null, updateWrapper);
